...
//		$(this).html('重新发送(60)s');
	...
	$("#query").click(function(e) {
		...
		if(code != "" && $("#verification").val() == code) {
			var data = {
				action: "GetPatientInfoLoginByNamePhone",
			...
			var succLogin = function(Data) {
				...
				if(data.Success) {
					$.toast(data.Msg);
					setTimeout(function() {
						localStorage.setItem("hospitalcode",data.Value.PatientInfoLoginTable[0].Barcode);
						localStorage.setItem('userName',data.Value.PatientInfoLoginTable[0].Barcode);
						localStorage.setItem('patientName',username);
						location.href = "PatientReport.html?loginType=name";
					},500);
				}else {
